...
int main()
{
  ...
  double Kp_init = 0.2f;
  double Ki_init = 0.002f;
  double Kd_init = 3.0f;
  pid.Init(Kp_init,Ki_init,Kd_init);
  PID speed_pid;
  speed_pid.Init(3.f,0.002f,2.f);
    ...
    if (length && length > 2 && data[0] == '4' && data[1] == '2')
    {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          double cte_speed = speed - speed_des;
          ...
          */
          ...
          // normalize the value
           double range = 2.f; // [-1, 1].
           steer_value = range / (1 + exp(-steer_value)) -1.0f;
